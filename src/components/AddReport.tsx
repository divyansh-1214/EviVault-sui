...
export default function AddReport({ packageId }: AddReportProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Create transaction
            tx.moveCall({
                target: `${packageId}::evidence_system::add_report`,
                arguments: [
                    tx.pure(caseNo),
                    tx.pure(firNo),
            ...
            setStatus({
                type: 'success',
                message: `Report added successfully! Transaction ID: ${result.digest}`
            });
            ...
            setStatus({
                type: 'error',
                message: `Error adding report: ${error instanceof Error ? error.message : String(error)}`
            });
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <h2 className="text-2xl font-semibold mb-6">Add New Report</h2>
